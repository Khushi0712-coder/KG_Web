...
const handlePay = async () => {
  try {
    const resp = await fetch(`${API_BASE_URL}/api/payment/create`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        amount,
    ...
    const cashfree = await load({ mode: "production" }); // ✅ live mode
    ...
    if (response?.error) {
      console.error("Cashfree Checkout Error:", response.error.message);
